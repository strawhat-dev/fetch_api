...
export const jsonify: (x: any) => string | undefined;
...
export interface FetchOptions extends Omit<RequestInit, 'method' | 'body' | 'headers'> {
  ...
  /** Define a default callback for resolved requests with sucessful responses, which may be used to perform some background task, or handle & intercept the returned response (by returning a _non-`undefined`_ value, where the resolved value will be returned instead). Asynchronous callbacks that return a `Promise` may be awaited and resolved if the callback is provided under the sub-property `onres.await`. */
